...
import { listGames, joinGame, getUser, ApiError, type Game } from "./api";
...
export default function Lobby({ token, onLogout, onOpenGame }: LobbyProps) {
  ...
  const [userMmr, setUserMmr] = useState<Record<number, number>>({});

  const refresh = useCallback(async () => {
    try {
      const allGames = await listGames(token);
      setGames(allGames);
      await loadMmrForGames(allGames);
      setError(null);
    } catch (err) {
  ...
  async function loadMmrForGames(games: Game[]) {
    const ids = new Set<number>();
    games.forEach(g => {
      if (g.player_white_id) ids.add(g.player_white_id);
      if (g.player_black_id) ids.add(g.player_black_id);
    });
    const entries = await Promise.all(
      Array.from(ids).map(id =>
        getUser(token, id).then(user => [id, user.mmr] as [number, number])
      )
    );
    setUserMmr(Object.fromEntries(entries));
  }
            ...
            <ul className="m-0 flex list-none flex-col gap-2 p-0">
              ...
              {searchOngoing.map((g) => (
                <GameCard
                  ...
                  game={g}
                  myUserId={myUserId}
                  playerWhiteMmr={g.player_white_id ? userMmr[g.player_white_id] : undefined}
                  playerBlackMmr={g.player_black_id ? userMmr[g.player_black_id] : undefined}
